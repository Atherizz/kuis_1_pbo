...
public class InisiasiObjek {
    public static void initiateObject(ArrayList<Kursus> daftarKursus, ArrayList<Konten> daftarKonten,
        ...
        // --- Java (mp1): k1, k2 ---
        k1.getDaftarKonten().add(new Video(nextKontenId++, "Video: Intro Java", k1, "Pengenalan Java & tooling", 600));
        k1.getDaftarKonten()
                .add(new Kuis(nextKontenId++, "Kuis: Variabel & Tipe Data", k1, "Dasar tipe data & operator", 12));
        ...
        k2.getDaftarKonten().add(
                new Video(nextKontenId++, "Video: Collections & Generics", k2, "List, Map, Set, generic type", 720));
        ...
        k3.getDaftarKonten().add(
                new Video(nextKontenId++, "Video: User Flow Dasar", k3, "Task flow & wireframe low-fidelity", 540));

        k4.getDaftarKonten()
                .add(new Video(nextKontenId++, "Video: Wireframe ke Prototype", k4, "Low-fi → hi-fi di Figma", 700));
        k4.getDaftarKonten()
                .add(new Kuis(nextKontenId++, "Kuis: Usability Metrics", k4, "SUS, task success, time-on-task", 8));

        // --- Mobile Development (mp3): k5, k6 ---
        k5.getDaftarKonten().add(
                new Video(nextKontenId++, "Video: Arsitektur Aplikasi Mobile", k5, "Activity/Fragment, MVVM", 660));
        ...
        k6.getDaftarKonten().add(new Video(nextKontenId++, "Video: Kotlin Dasar", k6, "Syntax, nullable, when", 620));
        k6.getDaftarKonten()
                .add(new Kuis(nextKontenId++, "Kuis: Kotlin Fundamentals", k6, "Null-safety, collections", 10));
        ...
        k7.getDaftarKonten()
                .add(new Video(nextKontenId++, "Video: JOINs Praktis", k7, "INNER/LEFT/RIGHT/FULL (simulasi)", 780));
        ...
        k8.getDaftarKonten()
                .add(new Video(nextKontenId++, "Video: Optimasi Query", k8, "Profiling, slow query log", 720));

        // --- Machine Learning (mp5): k9, k10 ---
        k9.getDaftarKonten()
                .add(new Video(nextKontenId++, "Video: Supervised vs Unsupervised", k9, "Task, dataset, metric", 900));
        ...
        k10.getDaftarKonten().add(new Artikel(nextKontenId++, "Artikel: Evaluasi Model", k10, "Accuracy vs F1, ROC-AUC",
                k10.getInstruktur().getNama()));
        k10.getDaftarKonten()
                .add(new Video(nextKontenId++, "Video: Confusion Matrix", k10, "TP/FP/TN/FN & interpretasi", 660));
    }
}
